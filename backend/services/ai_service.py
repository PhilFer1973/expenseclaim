...
- gross_amount is the amount the customer ACTUALLY PAID. If the receipt shows a discount, loyalty saving or meal-deal that reduces the bill (e.g. "TOTAL 7.40" then "TOTAL TO PAY 5.35"), use the LOWER amount actually paid — the "Total to pay" / "Amount due" / "Balance due" / the card or cash payment line — NOT the higher pre-discount subtotal.
...
- receipt_date must be in ISO-8601 (YYYY-MM-DD). Read dates carefully:
  * Find the purchase date wherever it appears: it may be labelled "Date:" near the top, OR printed in the FOOTER near the barcode / store-locator / transaction line. Supermarkets (e.g. Tesco) often print it at the very bottom right next to the time, e.g. "19/06/26  8:11". Use the actual purchase date. Do not mistake the card number, receipt/transaction number or the time itself for the date — but the date usually sits immediately beside the time.
